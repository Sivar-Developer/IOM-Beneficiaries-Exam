        ...
        <div class="col-lg-6 offset-lg-3">
            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    <form action="{{ route('beneficiaries.store') }}" method="POST" class="form-horizontal" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group row">
                            <label for="name" class="col-3 form-control-label{{ $errors->has('name') ? ' text-danger' : '' }}">Name</label>
                            <input type="text" class="col-9 form-control round{{ $errors->has('name') ? ' is-invalid' : '' }}" name="name" value="{{ old('name') }}">
                        </div>
                        <div class="form-group row">
                            <label for="birthdate" class="col-3 form-control-label{{ $errors->has('birthdate') ? ' text-danger' : '' }}">Birthdate</label>
                            <input type="date" class="col-9 form-control round{{ $errors->has('birthdate') ? ' is-invalid' : '' }}" name="birthdate" value="{{ old('birthdate')}}">
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-2 radio-custom radio-primary">
                                <input type="radio" id="{{ $gender }}" name="gender" value="{{ $gender }}" {{ old('gender') == $gender ? 'checked' : '' }} />
                        ...
                        <div class="form-group row">
                            ...
                            <input type="text" class="col-9 form-control round{{ $errors->has('phone_number') ? ' is-invalid' : '' }}" name="phone_number" value="{{ old('phone_number') }}">
                        </div>
                        <div class="form-group row">
                            ...
                            <input type="number" min="0" class="col-9 form-control round{{ $errors->has('national_id_number') ? ' is-invalid' : '' }}" name="national_id_number" value="{{ old('national_id_number') }}">
                        </div>
                        <div class="form-group row">
                            <label for="mother_name" class="col-3 form-control-label{{ $errors->has('mother_name') ? ' text-danger' : '' }}">Mother Name</label>
                            <input type="text" class="col-9 form-control round{{ $errors->has('mother_name') ? ' is-invalid' : '' }}" name="mother_name" value="{{ old('mother_name') }}">
                        </div>
                        <div class="form-group row">
                            ...
                            <select class="col-9 form-control round{{ $errors->has('martial_status') ? ' is-invalid' : '' }}" name="martial_status">
                                @foreach(['Single','Married','Divorced'] as $status)
                                    <option value="{{ $status }}" {{ old('martial_status') == $status ? 'selected' : '' }}>{{ $status }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="input-group round col-9">
                                ...
                                <input type="number" min="0" step="0.01" class="form-control round{{ $errors->has('monthly_income') ? ' is-invalid' : '' }}" name="monthly_income" value="{{ old('monthly_income') }}">
                            </div>
                        </div>
                        <div class="form-group row">
